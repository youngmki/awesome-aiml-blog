...
### 1.2. NLP
* [Some Remarks on LLMs (Jan 2023)](https://gist.github.com/yoavg/59d174608e92e845c8994ac2e234c8a9): 현재 사람 수준에서 섀넌의 추측 게임을 수행 가능할 정도로 LLM은 지능을 가지고 있음, 그러나 대규모 언어 모델링만으로 얻을 수 있는 것이 아님, 기존 언어 모델은 텍스트 기호만 학습했을 뿐 기호에 의미가 연결되어 있지 않음 ← 프로그래밍 언어 코드 데이터 훈련, 명시적인 지시를 통한 튜닝, RLHF 단계의 도입으로 변곡점 발생(= 기호와 의미의 연결이 가능) + 언어 모델 한계에 대한 진부한 주장들과 그와 별개로 아직은 실제로 제대로 하지 못한 것들
